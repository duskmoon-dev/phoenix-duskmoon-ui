...
- Centralized common JavaScript extension and DuskmoonBundler path defaults.
...
- `DuskmoonBundler.static_path/2` and `DuskmoonBundler.static_url/2` resolve DuskmoonBundler-managed scripts, stylesheets, and emitted image/font assets with Phoenix-style helper names.
- `mix duskmoon_bundler.build --tailwind` now writes a CSS manifest using the existing production manifest format.

### Deprecated

- `DuskmoonBundler.entry_path/2` is deprecated in favor of `DuskmoonBundler.static_path/2`.

### Fixed

- `mix duskmoon_bundler.js.check --type-aware --type-check` now fails on TypeScript compiler diagnostics such as `typescript/TS2322`.
- `mix duskmoon_bundler.build --tailwind` now honors `hash: false` from DuskmoonBundler config for Tailwind CSS output filenames, while keeping CLI `--hash`/`--no-hash` overrides.

## 0.13.1

### Fixed

- `mix duskmoon_bundler.js.check --type-aware` now forwards only `typescript/*` rules to `tsgolint`, matching Oxlint's type-aware rule namespace and avoiding crashes when normal oxlint category rules such as `"correctness"` are configured.
...
- `asset_url_prefix` config and `mix duskmoon_bundler.build --asset-url-prefix` for changing production JavaScript and CSS asset URLs without changing Phoenix output paths.
...
- `mix duskmoon_bundler.js.check` now supports `--type-aware` and `--type-check` for TypeScript-aware linting through `tsgolint`.
...
- Production builds now tree-shake JavaScript by default, with `tree_shaking: false` and `mix duskmoon_bundler.build --no-tree-shaking` available to preserve unused exports.
...
- DuskmoonBundler now dogfoods type-aware JavaScript checks in `mix lint`.

### Breaking changes

- Custom plugins that return `{:proxy, filename, opts}` from `prebundle_entry/1` must use `DuskmoonBundler.JS.PrebundleEntry.Import` and `DuskmoonBundler.JS.PrebundleEntry.Export` entries for `:imports` and `:exports`; plain map entries are no longer accepted.
- Custom `extract_imports/3` plugin callbacks must return `{:ok, %DuskmoonBundler.JS.ImportExtractor.Result{}}` instead of plain maps.
...
- JavaScript emitted by plugins and framework compilers now re-enters DuskmoonBundler's common post-processing pipeline.
...
- Named configuration profiles for multi-app and umbrella support. Use `config :duskmoon_bundler, :my_app_web, [...]` to define per-app configs, and pass the profile name to Mix tasks (`mix duskmoon_bundler.build my_app_web`) and the dev server plug (`plug DuskmoonBundler.DevServer, profile: :my_app_web`). The existing flat `config :duskmoon_bundler` format continues to work unchanged.

## 0.10.9

### Fixed

- `mix duskmoon_bundler.build` now compiles the project and starts only DuskmoonBundler's application, avoiding database connection attempts during asset builds in Phoenix projects while keeping DuskmoonBundler services available for Tailwind builds.

## 0.10.8

### Fixed

- `mix duskmoon_bundler.build` now compiles the project without starting the application, avoiding database connection attempts during asset builds in Phoenix projects.
...
- Dev server output now supports `import.meta.env` runtime access for modules that reference it, including `MODE`, `DEV`, `PROD`, and exposed `DUSKMOON_BUNDLER_*` values.

### Fixed

- `DuskmoonBundler.entry_path/2` now resolves production manifests written by `mix duskmoon_bundler.build` under `priv/static/assets/js`, returns `/assets/js/...` paths, and passes them through Phoenix `static_path/1` for `phx.digest` compatibility.
...
- Solid JSX/TSX support via `DuskmoonBundler.Plugin.Solid`. Runs `babel-preset-solid` through QuickBEAM — no Node.js required. Enable with `plugins: [DuskmoonBundler.Plugin.Solid]` in DuskmoonBundler config.
...
- Examples now use `DuskmoonBundler.Formatter` and `mix duskmoon_bundler.lint` with framework-appropriate plugins.
...
- `import_source` config (e.g. `"react"`) is now passed through to production builds, fixing JSX transform in `mix duskmoon_bundler.build`.
...
- `mix duskmoon_bundler.lint` no longer attempts to parse `.svelte` and `.vue` files as JavaScript.
- Build config `format:` is no longer clobbered by formatter config `config :duskmoon_bundler, :format`.
...
- `mix duskmoon_bundler.build` documentation now uses the supported `--sourcemap false` CLI form for disabling production source maps.
...
- `process.env.NODE_ENV` is now defined automatically from DuskmoonBundler's build mode.

## 0.9.0

### Added

- Added `DuskmoonBundler.entry_path/2` for resolving source entries in development and hashed manifest assets in production.
...
- Production entry paths now read DuskmoonBundler's manifest output through a first-class helper instead of requiring app-local layout helpers.
...
- `mix igniter.install duskmoon_bundler` now auto-detects `assets/js/app.js` vs `assets/js/app.ts` instead of hardcoding `.ts`.

## 0.8.3

### Fixed

- `mix igniter.install duskmoon_bundler` now fully removes legacy esbuild and tailwind configuration:
...
- Refactored `Mix.Tasks.DuskmoonBundler.Install` for readability: added module aliases, extracted predicate helpers, flattened nested control flow.

## 0.8.2

### Added

- `DuskmoonBundler.Formatter` — `mix format` plugin for JS/TS. Add `plugins: [DuskmoonBundler.Formatter]` to `.formatter.exs` and JS/TS files are formatted alongside Elixir with oxfmt.
- `mix igniter.install duskmoon_bundler` now adds `DuskmoonBundler.Formatter` to `.formatter.exs` automatically.
...
- Configurable file discovery via `sources:` and `ignore:` in `config :duskmoon_bundler`. Default sources: `["**/*.{js,ts,jsx,tsx,vue}"]`, default ignore: `["node_modules/**", "vendor/**"]`.

### Changed

- Unified file discovery across `duskmoon_bundler.js.format`, `duskmoon_bundler.js.check`, and `duskmoon_bundler.lint` — all use the same `sources` and `ignore` config.

## 0.8.0

### Added

- `mix duskmoon_bundler.js.format` — format JS/TS assets with oxfmt via NIF. No Node.js required.
- `mix duskmoon_bundler.js.check` — format check + lint in one command via NIF.
- `mix duskmoon_bundler.install` — Igniter-based project setup. Adds DuskmoonBundler config, dev server plug, watcher, removes esbuild/tailwind deps. Migrates existing Prettier/oxfmt JSON config.
- `config :duskmoon_bundler, :format` — Elixir-native format config (falls back to `.oxfmtrc.json`).
...
- File discovery uses `config :duskmoon_bundler, :root` consistently across format, check, and lint tasks.
- Renamed `mix duskmoon_bundler.js.fmt` → `mix duskmoon_bundler.js.format`.

## 0.7.1

- Improve `mix duskmoon_bundler.lint` output to match credo style — grouped by category, severity tags, edge markers, summary by category

## 0.7.0

### Added

- `mix duskmoon_bundler.lint` — lint JS/TS/JSX/TSX/Vue assets with oxlint's 650+ built-in rules via NIF. No Node.js required. Configurable plugins, rules, and custom Elixir lint rules via `config :duskmoon_bundler, :lint`.
...
DuskmoonBundler automatically reads `compilerOptions.paths` from `tsconfig.json` in the
...
- Reorganize internal modules into `DuskmoonBundler.JS.*`, `DuskmoonBundler.CSS.*`, `DuskmoonBundler.Dev.*` namespaces
...
loader powered by QuickBEAM. DuskmoonBundler now resolves and prebundles any Tailwind
...
- Delete `DuskmoonBundler.PackageResolver` — delegate to `NPM.PackageResolver`
...
- Centralize specifier predicates (`relative?`, `absolute?`, `bare?`, `node_builtin?`)
  in `Builder.Resolver`
- Extract `DuskmoonBundler.Extensions` as single source for file extension lists
...
- Extract shared Mix task helpers into `DuskmoonBundler.JsHelpers`
- Rename `Builder.Assets` to `Builder.Writer` to avoid collision with `DuskmoonBundler.Assets`
...
`DuskmoonBundler.JSAsset.read!/1` loads them at runtime.

### Maintainer Tooling

- `mix duskmoon_bundler.js.check` — run oxfmt format check and oxlint via npx
- `mix duskmoon_bundler.js.fmt` — format TypeScript assets via npx
...
```elixir
config :duskmoon_bundler, external: ["vue"]
# import { ref } from 'vue'  →  const { ref } = Vue;

config :duskmoon_bundler, external: %{"vue" => "MyVue"}
...
```bash
mix duskmoon_bundler.build --entry index.html
```
...
New `DuskmoonBundler.Preload.tags/2` generates `<link rel="modulepreload">` tags
...
All config now lives under `config :duskmoon_bundler` in your standard `config/*.exs` files:
...
`DuskmoonBundler.Plugin` behaviour with resolve, load, transform, render_chunk hooks.
...
Split into `DuskmoonBundler.Builder.Resolver`, `DuskmoonBundler.Builder.Collector`,
`DuskmoonBundler.Builder.Output`, and `DuskmoonBundler.ChunkGraph`.
