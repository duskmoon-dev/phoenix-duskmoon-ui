defmodule DuskmoonBundler.JsCheckTest do
  ...
  setup do
    ...
    original_sources = Application.get_env(:duskmoon_bundler, :sources)
    ...
    on_exit(fn ->
      ...
      if original_sources,
        do: Application.put_env(:duskmoon_bundler, :sources, original_sources),
        else: Application.delete_env(:duskmoon_bundler, :sources)
    end)
  ...
  test "reports parser errors in syntax lint mode" do
    ...
    output =
      capture_io(:stderr, fn ->
        catch_exit(Mix.Tasks.DuskmoonBundler.Js.Check.run([]))
      end)
  ...
  test "type-aware check reports tsgolint diagnostics" do
    ...
    output =
      capture_io(:stderr, fn ->
        catch_exit(Mix.Tasks.DuskmoonBundler.Js.Check.run(["--type-aware"]))
      end)
  ...
  test "type-check diagnostics are promoted to errors" do
    ...
    assert %{severity: :deny} =
             DuskmoonBundler.JS.Check.promote_type_check_diagnostic(diagnostic, type_check: true)
  end
  ...
  test "type-aware check forwards only typescript rules to tsgolint" do
    ...
    capture_io(fn ->
      Mix.Tasks.DuskmoonBundler.Js.Check.run(["--type-aware"])
    end)
  ...
  test "type-aware check submits framework single-file component scripts as virtual files" do
    ...
    Application.put_env(:duskmoon_bundler, :sources, ["**/*.{js,ts,jsx,tsx,vue,svelte}"])
    Application.put_env(:duskmoon_bundler, :lint, tsgolint: tsgolint, rules: %{})
