defmodule DuskmoonBundler.PipelineTest do
  ...
  describe "compile/3 with TypeScript" do
    test "strips types and returns sourcemap" do
      {:ok, result} = DuskmoonBundler.Pipeline.compile("app.ts", "const x: number = 42")
    ...
    test "compiles JSX" do
      {:ok, result} = DuskmoonBundler.Pipeline.compile("app.tsx", "<div />")
      assert result.code =~ "jsx"
    end

    test "applies target downleveling" do
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile("app.js", "const x = a ?? b", target: :es2019)
    ...
    test "injects import.meta.env defines" do
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile(
          "app.ts",
    ...
    test "supports runtime access to injected import.meta.env" do
      ...
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile("app.ts", source,
    ...
    test "supports destructuring import.meta.env" do
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile(
          "app.ts",
          "const { MODE } = import.meta.env; console.log(MODE)",
          define: %{"import.meta.env.MODE" => ~s("development")}
        )
    ...
    test "supports optional access to import.meta.env" do
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile("app.ts", "console.log(import.meta?.env?.MODE)",
          define: %{"import.meta.env.MODE" => ~s("development")}
        )
    ...
    test "leaves non import.meta.env defines unchanged" do
      ...
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile("app.js", source,
  ...
  describe "compile/3 with Vue SFC" do
    test "compiles a simple SFC" do
      ...
      {:ok, result} = DuskmoonBundler.Pipeline.compile("App.vue", source)
    ...
    test "applies JavaScript postprocess transforms to compiled SFC output" do
      dir =
        Path.join(
          System.tmp_dir!(),
          "duskmoon_bundler-vue-postprocess-#{System.unique_integer([:positive])}"
        )
      ...
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile(Path.join(dir, "App.vue"), source,
          define: %{"import.meta.env.MODE" => ~s("development")}
        )
  ...
  describe "compile/3 with Svelte" do
    test "applies JavaScript postprocess transforms to compiled Svelte output" do
      dir =
        Path.join(
          System.tmp_dir!(),
          "duskmoon_bundler-svelte-postprocess-#{System.unique_integer([:positive])}"
        )
      ...
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile(Path.join(dir, "App.svelte"), source,
          define: %{"import.meta.env.MODE" => ~s("development")}
        )
  ...
  describe "compile/3 with plugin output" do
    test "applies JavaScript postprocess transforms after custom plugin compilation" do
      dir =
        Path.join(
          System.tmp_dir!(),
          "duskmoon_bundler-plugin-postprocess-#{System.unique_integer([:positive])}"
        )
      ...
      assert {:ok, result} =
               DuskmoonBundler.Pipeline.compile(Path.join(dir, "component.future"), "ignored",
    ...
    test "plugins can compile custom file types" do
      ...
      assert {:ok, %{code: ~s(export default "hello")}} =
               DuskmoonBundler.Pipeline.compile("component.custom", "hello",
    ...
    test "plugins receive compiled output" do
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile("app.ts", "const x: number = 42",
  ...
  describe "compile/3 with CSS" do
    test "passes through CSS" do
      {:ok, result} = DuskmoonBundler.Pipeline.compile("app.css", ".foo { color: red }")
      assert result.code =~ "color"
    end

    test "minifies CSS" do
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile("app.css", ".foo {\n  color: red;\n}", minify: true)
    ...
    test "inlines @import from disk" do
      ...
      {:ok, result} = DuskmoonBundler.Pipeline.compile(path, File.read!(path))
  ...
  describe "compile/3 with import rewriting" do
    ...
    test "skips rewriting when no rewrite_import given" do
      {:ok, result} =
        DuskmoonBundler.Pipeline.compile("app.ts", "import { ref } from 'vue'\nref(0)")
  ...
  describe "compile/3 with JSON" do
    test "wraps JSON as ES module" do
      {:ok, result} = DuskmoonBundler.Pipeline.compile("data.json", ~s({"key":"value"}))
  ...
  describe "compile/3 with CSS Modules" do
    test "compiles .module.css to JS + scoped CSS" do
      {:ok, result} = DuskmoonBundler.Pipeline.compile("btn.module.css", ".btn { color: red }")
      assert result.code =~ "export default"
  ...
  describe "compile/3 errors" do
    test "returns error for unsupported extensions" do
      {:error, {:unsupported, ".xyz"}} = DuskmoonBundler.Pipeline.compile("data.xyz", "binary")
    end

    test "returns error for invalid TypeScript" do
      {:error, errors} = DuskmoonBundler.Pipeline.compile("bad.ts", "const = ;")
