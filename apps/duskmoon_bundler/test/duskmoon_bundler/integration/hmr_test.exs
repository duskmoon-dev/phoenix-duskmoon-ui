defmodule DuskmoonBundler.Integration.TestPlug do
  @moduledoc false
  @behaviour Plug
...
defmodule DuskmoonBundler.Integration.HMRTest do
  ...
  setup %{browser: browser} do
    ...
    DuskmoonBundler.HMR.GlobGraph.clear()
    ...
    <!DOCTYPE html>
    <html>
    <head><title>DuskmoonBundler HMR Test</title></head>
    <body>
    ...
    el.id = 'duskmoon_bundler-test'
    el.textContent = 'hello from duskmoon_bundler'
    document.body.appendChild(el)
    """)

    dev_server_opts = DuskmoonBundler.DevServer.init(root: @fixture_dir, prefix: "/assets")
    plug_opts = %{root: @fixture_dir, dev_server: dev_server_opts}

    {:ok, server} =
      Bandit.start_link(
        plug: {DuskmoonBundler.Integration.TestPlug, plug_opts},
        port: @port,
        ip: :loopback,
        startup_log: false
      )
  ...
  describe "dev server module serving" do
    ...
    test "rewrites relative imports between modules", %{frame: frame} do
      ...
      window.__duskmoon_bundlerImportResult = greeting
      """)
    ...
    test "imports CSS from JavaScript and injects a style tag", %{frame: frame} do
      ...
      const el = document.createElement('div')
      el.className = 'duskmoon_bundler-css-import-target'
      el.textContent = 'styled'
      document.body.appendChild(el)
      window.__duskmoon_bundlerCssImportLoaded = true
      """)
    ...
    test "parent modules accept dependency updates without reloading", %{frame: frame} do
      ...
      const store = (window.__duskmoon_bundlerAcceptedDep ??= { events: [] })
    ...
    test "parent modules accept multiple dependency updates", %{frame: frame} do
      ...
      const store = (window.__duskmoon_bundlerMultiDep ??= { events: [] })
      store.reloads = (store.reloads ?? 0) + 1
    ...
    test "imports CSS modules from JavaScript with exports and injected styles", %{frame: frame} do
      ...
      document.body.appendChild(el)
      window.__duskmoon_bundlerCssModuleClass = classes.btn
      """)
