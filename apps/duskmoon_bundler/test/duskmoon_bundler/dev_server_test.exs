defmodule DuskmoonBundler.DevServerTest do
  ...
  setup do
    ...
    DuskmoonBundler.HMR.ModuleGraph.clear()
  ...
  describe "CSS files" do
    ...
    test "serves CSS imports as JavaScript modules" do
      ...
      assert conn.status == 200
      assert conn.resp_body =~ "__duskmoon_bundler_updateStyle"
      assert conn.resp_body =~ "color"
      assert get_resp_header(conn, "content-type") |> hd() =~ "javascript"
    end
    ...
    test "serves raw CSS and CSS import modules from separate cache entries" do
      ...
      assert import_conn.resp_body =~ "__duskmoon_bundler_updateStyle"
      assert get_resp_header(import_conn, "content-type") |> hd() =~ "javascript"

      assert raw_conn.resp_body =~ "color: red"
      refute raw_conn.resp_body =~ "__duskmoon_bundler_updateStyle"
  ...
  describe "caching" do
    test "records served modules in HMR module graph" do
      ...
      node = DuskmoonBundler.HMR.ModuleGraph.get_by_url("/assets/app.ts")
      dep = DuskmoonBundler.HMR.ModuleGraph.get_by_url("/assets/dep.ts")
    ...
    test "records CSS import query variants separately in HMR module graph" do
      ...
      nodes =
        DuskmoonBundler.HMR.ModuleGraph.get_by_file(Path.join(@fixture_dir, "src/style.css"))
    ...
    test "watcher invalidation does not replace dev-server cache" do
      ...
      watcher =
        start_supervised!(
          {DuskmoonBundler.Watcher, root: source_dir, name: :test_dev_server_watcher_cache}
        )
    ...
    test "watcher compares changed files against previous cached hashes" do
      ...
      watcher =
        start_supervised!(
          {DuskmoonBundler.Watcher, root: source_dir, name: :test_hmr_cached_hashes}
        )

      send(watcher, {:rebuild, vue_path})

      assert_receive {:duskmoon_bundler_hmr, :update, %{path: "App.vue", changes: [:style]}}, 1000
    end
    ...
    test "watcher invalidation evicts CSS import cache" do
      ...
      watcher =
        start_supervised!(
          {DuskmoonBundler.Watcher, root: source_dir, name: :test_css_import_cache}
        )
    ...
    test "watcher invalidation evicts cache even for never-served files" do
      ...
      watcher =
        start_supervised!({DuskmoonBundler.Watcher, root: source_dir, name: :test_never_served})
    ...
    test "compilation error does not serve stale cache" do
      ...
      watcher =
        start_supervised!({DuskmoonBundler.Watcher, root: source_dir, name: :test_compile_error})
  ...
  describe "non-matching paths" do
    ...
    test "serves asset script fetches as JavaScript modules" do
      ...
      conn =
        ...
        |> DuskmoonBundler.DevServer.call(opts)
  ...
  describe "error handling" do
    test "returns 500 with error overlay for invalid source" do
      ...
      assert conn.status == 500
      assert conn.resp_body =~ "[DuskmoonBundler] Compilation error"
    end
  end
