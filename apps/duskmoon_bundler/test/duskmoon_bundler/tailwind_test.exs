defmodule DuskmoonBundler.TailwindTest do
  use ExUnit.Case
  ...
  describe "build/1" do
    test "generates CSS from source files" do
      {:ok, css} =
        DuskmoonBundler.Tailwind.build(
          sources: [%{base: @fixture_dir, pattern: "**/*.{html,heex}"}]
        )
    ...
    test "minifies output" do
      {:ok, normal} =
        DuskmoonBundler.Tailwind.build(sources: [%{base: @fixture_dir, pattern: "**/*.html"}])
  ...
  describe "rebuild/2" do
    test "returns :unchanged when no new candidates" do
      ...
      assert :unchanged =
               DuskmoonBundler.Tailwind.rebuild([
