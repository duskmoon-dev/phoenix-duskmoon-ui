...
DuskmoonBundler's asset pipeline is for files referenced by JavaScript or CSS when you want dependency tracking, hashing, and URL rewriting. Once an asset is part of the DuskmoonBundler graph, templates can reference it by its source-relative path:
...
For Vite migration compatibility, DuskmoonBundler also supports an optional `public_dir`. It is disabled by default. When enabled, files in that directory are served and copied without transformation:
