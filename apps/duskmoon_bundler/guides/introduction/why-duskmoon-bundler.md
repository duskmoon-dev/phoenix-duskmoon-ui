...
DuskmoonBundler takes a different approach: compile frontend assets natively inside the BEAM using Rust NIFs.
...
Because DuskmoonBundler runs inside your Phoenix endpoint as a Plug, it serves assets on the same port
...
When you edit a `.heex` template, DuskmoonBundler detects the new Tailwind classes, incrementally
