...
- Adds the `DuskmoonBundler.DevServer` plug to your endpoint
- Adds the DuskmoonBundler watcher to `config/dev.exs`
...
Add DuskmoonBundler to your dependencies:
...
The watcher starts `mix duskmoon_bundler.dev` automatically when `mix phx.server` runs, watching for file changes and triggering HMR updates and Tailwind rebuilds.
...
Add `DuskmoonBundler.Formatter` to `.formatter.exs`:
