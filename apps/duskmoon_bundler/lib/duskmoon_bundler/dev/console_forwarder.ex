defmodule DuskmoonBundler.Dev.ConsoleForwarder do
  @moduledoc false

  require Logger

  @endpoint "/@duskmoon_bundler/console"
  ...
  def inject(code) when is_binary(code) do
    DuskmoonBundler.JS.Asset.compiled!("dev/console-forwarder.ts") <> "\n" <> code
  end
  ...
  def log(%{"level" => level, "args" => args, "source" => source}) when is_list(args) do
    ...
    prefix =
      if source in [nil, ""],
        do: "[DuskmoonBundler][browser]",
        else: "[DuskmoonBundler][browser][#{source}]"
