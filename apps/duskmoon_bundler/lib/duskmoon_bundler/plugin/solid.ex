defmodule DuskmoonBundler.Plugin.Solid do
  @moduledoc """
  Solid JSX/TSX support for DuskmoonBundler.

  Solid JSX requires the Solid compiler rather than a generic JSX runtime
  transform. This plugin runs `babel-preset-solid` through DuskmoonBundler's JavaScript
  runtime and leaves normal module resolution to DuskmoonBundler. Add `DuskmoonBundler.Plugin.Solid`
  to `config :duskmoon_bundler, :plugins` when using Solid TSX, since `.jsx` and `.tsx`
  ...
  def prebundle_entry("solid-js") do
    {:proxy, "solid-js.js",
     exports: [
       DuskmoonBundler.JS.PrebundleEntry.Export.all_from("solid-js"),
       DuskmoonBundler.JS.PrebundleEntry.Export.all_from("solid-js/web")
     ]}
  end
        ...
        with {:ok, code, downlevelled?} <- maybe_downlevel(code, filename, opts) do
          {:ok,
           %DuskmoonBundler.Pipeline.Result{
             ...
             css: nil,
             hashes: %DuskmoonBundler.Pipeline.Result.Hashes{
               template: nil,
               style: nil,
               script: hash(source)
             }
  ...
  defp compile_filename(path, opts) do
    path
    |> Path.basename()
    |> DuskmoonBundler.JS.Extensions.apply_loader(Keyword.get(opts, :loaders, %{}))
  end
  ...
  defp extract_typed_imports(source, filename) do
    DuskmoonBundler.JS.ImportExtractor.extract_typed(source, filename,
  ...
  defp hash(value), do: DuskmoonBundler.Plugin.Helpers.cache_hash(value)
end
