defmodule DuskmoonBundler.Watcher do
  @moduledoc """
  ...
  def init(opts) do
    ...
    file_system_backend =
      Keyword.get(
        opts,
        :file_system_backend,
        Application.get_env(:duskmoon_bundler, :file_system_backend)
      )
    ...
    file_system_interval =
      Keyword.get(
        opts,
        :file_system_interval,
        Application.get_env(:duskmoon_bundler, :file_system_interval, @fallback_poll_interval)
      )
    ...
    fs_pids =
      Enum.map(all_dirs, fn dir ->
        {:ok, pid} = start_file_system(dir, file_system_backend, file_system_interval)
  ...
  defp start_file_system(dir, backend, interval) do
    case FileSystem.start_link(file_system_opts(dir, backend, interval)) do
      {:ok, _pid} = ok ->
        ok

      :ignore when is_nil(backend) ->
        FileSystem.start_link(file_system_opts(dir, :fs_poll, interval))
  ...
  defp file_system_opts(dir, nil, _interval), do: [dirs: [dir]]
  ...
  defp initial_tailwind_build(dirs, css_path, outdir) do
    ...
    case DuskmoonBundler.Tailwind.build(sources: sources, css: css_input, css_base: css_base) do
      {:ok, css} ->
        ...
        Logger.debug("[DuskmoonBundler] Initial Tailwind build: #{byte_size(css)} bytes")

      {:error, reason} ->
        Logger.warning("[DuskmoonBundler] Initial Tailwind build failed: #{inspect(reason)}")
    end
  end
  ...
  defp handle_js_change(path, state) do
    ...
    old_entry = DuskmoonBundler.Cache.get_file(path)
    DuskmoonBundler.Cache.evict_file(path)
    DuskmoonBundler.HMR.ModuleGraph.invalidate_file(path)

    case File.read(path) do
      {:ok, source} ->
        case DuskmoonBundler.Pipeline.compile(path, source, Map.to_list(state.config)) do
          {:ok, result} ->
            DuskmoonBundler.HMR.GlobGraph.update_from_source(path, source)
            DuskmoonBundler.HMR.ImportGraph.update_from_compiled(path, result.code)
      ...
      {:error, reason} when reason in [:enoent, :eacces, :eperm] ->
        DuskmoonBundler.HMR.ImportGraph.remove(path)
        DuskmoonBundler.HMR.GlobGraph.remove(path)
        DuskmoonBundler.HMR.ModuleGraph.remove_file(path)
  ...
  defp broadcast_glob_dependents(path, root) do
    path
    |> DuskmoonBundler.HMR.GlobGraph.dependents()
    |> Enum.reject(&(&1 == path))
    |> Enum.each(fn importer ->
      DuskmoonBundler.Cache.evict_file(importer)
  ...
  defp broadcast_change(path, relative, changes, root) do
    cond do
      ...
      true ->
        ...
        case DuskmoonBundler.HMR.Boundary.find_boundary(path, read_source) do
          {:ok, boundary_path} ->
  ...
  defp handle_tailwind_rebuild(changed_paths, state) do
    ...
    case DuskmoonBundler.Tailwind.rebuild(changed, css: css_input, css_base: css_base) do
      {:ok, css} ->
        ...
        Logger.debug("[DuskmoonBundler] Tailwind rebuilt (#{byte_size(css)} bytes)")
  ...
  defp broadcast(type, payload) do
    Registry.dispatch(DuskmoonBundler.HMR.Registry, :clients, fn entries ->
      msg = {:duskmoon_bundler_hmr, type, payload}
  ...
  def terminate(_reason, state) do
    Enum.each(state.fs_pids, &stop_file_system/1)
    :ok
  end

  defp stop_file_system(pid) do
    pid
    |> file_system_backend_pid()
    |> stop_process()

    stop_process(pid)
  end

  defp file_system_backend_pid(pid) do
