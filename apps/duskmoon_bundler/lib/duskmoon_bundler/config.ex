defmodule DuskmoonBundler.Config do
  @moduledoc """
  Read DuskmoonBundler configuration from application environment.

  ## Flat config (single app)

  All config lives under the `:duskmoon_bundler` application key in `config/config.exs`:
  ...
  Use a named profile to configure multiple independent DuskmoonBundler instances:
  ...
  Pass the profile name to Mix tasks and the plug:
      ...
      mix duskmoon_bundler.dev my_app_web --tailwind

      plug DuskmoonBundler.DevServer, root: "assets", profile: :my_app_web
  ...
  client code through `import.meta.env`. It defaults to `"DUSKMOON_BUNDLER_"` and accepts a
  ...
  def build(profile, overrides) do
    flat_env =
      Application.get_all_env(:duskmoon_bundler)
      |> Keyword.take(@build_keys)
      |> Keyword.reject(fn {k, v} -> k == :format and not is_atom(v) end)

    profile_env =
      if profile do
        Application.get_env(:duskmoon_bundler, profile, [])
        |> Keyword.take(@build_keys)
        |> Keyword.reject(fn {k, v} -> k == :format and not is_atom(v) end)
      else
        []
    ...
    config =
      ...
      |> then(&struct!(DuskmoonBundler.Config.Build, &1))

    tsconfig_paths = DuskmoonBundler.JS.TSConfig.discover_paths()
  ...
  def server(profile, overrides) do
    global_env = Application.get_env(:duskmoon_bundler, :server, [])

    profile_env =
      if profile do
        Application.get_env(:duskmoon_bundler, profile, []) |> Keyword.get(:server, [])
      else
        []
      end
    ...
    |> then(&struct!(DuskmoonBundler.Config.Server, &1))
  end
  ...
  def tailwind(nil), do: Application.get_env(:duskmoon_bundler, :tailwind, [])

  def tailwind(profile) when is_atom(profile) do
    profile_config = Application.get_env(:duskmoon_bundler, profile, [])

    Keyword.get(profile_config, :tailwind) ||
      Application.get_env(:duskmoon_bundler, :tailwind, [])
  end
end
