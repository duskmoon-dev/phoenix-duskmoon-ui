defmodule DuskmoonBundler.JS.Runtime do
  @moduledoc """
  Runs JavaScript build tools through QuickBEAM with package dependencies isolated
  from the user's application.
  """

  alias DuskmoonBundler.JS.Runtime.Bundler
  alias DuskmoonBundler.JS.Runtime.Entry
  alias DuskmoonBundler.JS.Runtime.Error
  ...
  defp build_define(opts, runtime) do
    ...
    Map.merge(
      %{
        "DUSKMOON_BUNDLER_JS_RUNTIME_ROOT" => runtime.install_dir,
        "DUSKMOON_BUNDLER_NODE_MODULES" => runtime.node_modules
      },
      define
    )
  end
  ...
  defp verify_named_runtime_signature!(name, opts) do
    ...
    case :persistent_term.get(named_runtime_key(name), nil) do
      ...
      _other ->
        raise ArgumentError,
              "named DuskmoonBundler JS runtime #{inspect(name)} already started with different options"
    end
  end
