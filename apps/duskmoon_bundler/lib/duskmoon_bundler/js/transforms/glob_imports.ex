defmodule DuskmoonBundler.JS.Transforms.GlobImports do
  @moduledoc """
  ...
  alias DuskmoonBundler.JS.{AST, Patch}
  ...
  defp parse_glob_args([pattern_node | rest]) do
    with {:ok, patterns} <- parse_patterns(pattern_node),
         {:ok, %DuskmoonBundler.JS.Transforms.GlobImports.Call{} = opts} <- parse_options(rest) do
  ...
  defp parse_options([]), do: {:ok, %DuskmoonBundler.JS.Transforms.GlobImports.Call{}}

  defp parse_options([%{type: :object_expression, properties: props} | _]) do
    opts = %DuskmoonBundler.JS.Transforms.GlobImports.Call{}
  ...
  defp import_path(file, %{query: query}), do: DuskmoonBundler.URL.append_query(file, query)
