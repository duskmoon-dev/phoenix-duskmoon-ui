defmodule DuskmoonBundler.JS.Transforms.Imports do
  @moduledoc """
  ...
  ## Examples
      ...
      iex> DuskmoonBundler.JS.Transforms.Imports.rewrite(source, "test.ts", fn
  ...
  def rewrite(source, filename, rewrite_fn) do
    case OXC.parse(source, filename) do
      {:ok, ast} ->
        ...
        {:ok, DuskmoonBundler.JS.Patch.apply(source, patches)}
  ...
  ## Examples
      ...
      iex> DuskmoonBundler.JS.Transforms.Imports.rewrite_map(source, "test.ts", %{
      ...>   "vue" => "/@vendor/vue.js",
  ...
  defp maybe_patch(source_node, rewrite_fn, patches) do
    case DuskmoonBundler.JS.AST.string_literal_span(source_node) do
      {:ok, specifier, s, e} ->
          ...
          {:rewrite, new_specifier} ->
            [
              DuskmoonBundler.JS.Patch.new(
                s,
                e,
                DuskmoonBundler.JS.AST.string_literal(new_specifier)
              )
              | patches
            ]
