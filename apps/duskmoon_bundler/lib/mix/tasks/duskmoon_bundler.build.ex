defmodule Mix.Tasks.DuskmoonBundler.Build do
  ...
  Reads configuration from `config :duskmoon_bundler` in your config files.
  ...
  def run(args) do
    ...
    Application.ensure_all_started(:duskmoon_bundler)
    ...
    config = DuskmoonBundler.Config.build(profile)
    tailwind_config = DuskmoonBundler.Config.tailwind(profile)
  ...
  defp build_js(opts) do
    ...
    {us, result} = :timer.tc(fn -> DuskmoonBundler.Builder.build(opts) end)
    ms = div(us, 1000)
  ...
  defp build_tailwind(parsed, tailwind_config, outdir, minify, config_hash, root) do
      ...
      case cli_sources do
        [] ->
          tailwind_config[:sources] ||
            [
              %{base: DuskmoonBundler.Paths.lib(), pattern: "**/*.{ex,heex}"},
              %{base: DuskmoonBundler.Paths.assets_dir(), pattern: "**/*.{vue,ts,tsx,js,jsx}"}
            ]
    ...
    case result do
      {:ok, %{css: %{path: path, size: size}, manifest: manifest}} ->
        ...
        DuskmoonBundler.Builder.Writer.write_manifest(css_outdir, manifest)

        Mix.shell().info("  #{Path.basename(path)}  #{DuskmoonBundler.Format.format_size(size)}")
        Mix.shell().info("  manifest.json  #{map_size(manifest)} entries")
  ...
  defp format_file(path) do
    DuskmoonBundler.Format.format_with_gzip(File.read!(path))
  end
end
