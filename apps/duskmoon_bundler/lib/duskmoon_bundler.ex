defmodule DuskmoonBundler do
  ...
  Add DuskmoonBundler to your Phoenix endpoint as a Plug:
  ...
  Or use the Mix tasks:
      ...
      mix duskmoon_bundler.build       # Production build
  """
  ...
  `DuskmoonBundler.DevServer`. In production, DuskmoonBundler reads `manifest.json` and returns the
  ...
  def static_path(conn_or_socket_or_endpoint_or_uri, path, overrides \\ []) do
    ...
    build = DuskmoonBundler.Config.build(profile, build_overrides)
    server = DuskmoonBundler.Config.server(profile, build_overrides)
