defmodule PhoenixDuskmoon.Component.Form.SwitchTest do
  ...
  test "renders basic switch" do
    result = render_component(&dm_switch/1, %{
      id: "test-switch",
      name: "notifications",
    ...
    assert result =~ ~s[<input type="checkbox" name="notifications" id="test-switch" value="true" class="sr-only peer">]
    assert result =~ ~s[<div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer]
