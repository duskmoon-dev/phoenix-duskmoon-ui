defmodule PhoenixDuskmoon.CssArt.SignatureTest do
  ...
  test "renders signature with required id" do
    result = render_component(&dm_art_signature/1, %{id: "sig-1"})

    assert result =~ "dm-art-signature"
    assert result =~ ~s[id="sig-1"]
  end

  test "renders signature with default content A" do
    result = render_component(&dm_art_signature/1, %{id: "sig-2"})

    assert result =~ ~s[data-content="A"]
  end

  test "renders signature with custom content" do
    result = render_component(&dm_art_signature/1, %{id: "sig-3", content: "Approved"})

    assert result =~ ~s[data-content="Approved"]
  end

  test "renders signature with small size" do
    result = render_component(&dm_art_signature/1, %{id: "sig-4", size: "small"})

    assert result =~ "dm-art-signature"
  end

  test "renders signature with large size" do
    result = render_component(&dm_art_signature/1, %{id: "sig-5", size: "large"})

    assert result =~ "dm-art-signature"
  end

  test "renders signature with custom color" do
    result = render_component(&dm_art_signature/1, %{id: "sig-6", color: "#00ff00"})

    assert result =~ "#00ff00"
  end

  test "renders signature with custom rotation" do
    result = render_component(&dm_art_signature/1, %{id: "sig-7", rotation: -45})

    assert result =~ "-45deg"
  end

  test "renders signature with custom opacity" do
    result = render_component(&dm_art_signature/1, %{id: "sig-8", opacity: 0.8})

    assert result =~ "0.8"
  end

  test "renders signature with custom position" do
    result = render_component(&dm_art_signature/1, %{id: "sig-9", position: "relative"})
  ...
  test "renders signature with custom right and top" do
    result =
      render_component(&dm_art_signature/1, %{id: "sig-10", right: "4rem", top: "4rem"})

    assert result =~ "4rem"
  end

  test "renders signature with custom class" do
    result = render_component(&dm_art_signature/1, %{id: "sig-11", class: "my-stamp"})
  ...
  test "renders signature with rest attributes" do
    result =
      render_component(&dm_art_signature/1, %{
        id: "sig-12",
  ...
  test "renders signature with medium size and correct pixel value" do
    result = render_component(&dm_art_signature/1, %{id: "sig-m"})
  ...
  test "renders signature with various position values" do
    for position <- ~w(fixed relative sticky static) do
      result = render_component(&dm_art_signature/1, %{id: "sig-p", position: position})
  ...
  test "renders signature with zero rotation" do
    result = render_component(&dm_art_signature/1, %{id: "sig-z", rotation: 0})
    assert result =~ "--rotate: 0deg"
  end

  test "renders signature with opacity edge cases" do
    for opacity <- [0.0, 1.0, 0.001] do
      result = render_component(&dm_art_signature/1, %{id: "sig-o", opacity: opacity})
      assert result =~ "--opacity: #{opacity}"
    end
  end

  test "renders signature with data-content attribute" do
    result = render_component(&dm_art_signature/1, %{id: "sig-dc", content: "OK"})

    assert result =~ ~s[data-content="OK"]
  end

  test "renders signature with dm-art-signature class" do
    result = render_component(&dm_art_signature/1, %{id: "sig-cls"})
  ...
  test "renders signature with relative position override" do
    result = render_component(&dm_art_signature/1, %{id: "sig-pos2", position: "relative"})

    assert result =~ "position: relative"
  end

  test "renders signature with negative rotation" do
    result = render_component(&dm_art_signature/1, %{id: "sig-neg", rotation: -90})

    assert result =~ "--rotate: -90deg"
  end

  test "renders signature with large rotation" do
    result = render_component(&dm_art_signature/1, %{id: "sig-360", rotation: 360})

    assert result =~ "--rotate: 360deg"
  end

  test "renders signature with custom right value" do
    result = render_component(&dm_art_signature/1, %{id: "sig-r", right: "50%"})

    assert result =~ "--right: 50%"
  end

  test "renders signature with custom top value" do
    result = render_component(&dm_art_signature/1, %{id: "sig-t", top: "0"})

    assert result =~ "--top: 0"
  end

  test "renders signature with default color" do
    result = render_component(&dm_art_signature/1, %{id: "sig-dc"})

    assert result =~ "--sign-color: #ff0000aa"
  end

  test "renders signature with emoji content" do
    result = render_component(&dm_art_signature/1, %{id: "sig-emoji", content: "✓"})

    assert result =~ ~s[data-content="✓"]
  end

  test "renders signature with aria-hidden for decorative content" do
    result = render_component(&dm_art_signature/1, %{id: "sig-a11y"})
